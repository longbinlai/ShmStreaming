...
hadoop jar /home/epcc/robeen/hadoop-0.21.0-streaming.jar \
  ...
  -jobconf stream.shm.map.input.unlock.empty.semaphore=64 \
  -jobconf stream.shm.map.input.unlock.full.semaphore=64 \
  -jobconf stream.shm.map.output.unlock.empty.semaphore=128 \
  -jobconf stream.shm.map.output.unlock.full.semaphore=128
