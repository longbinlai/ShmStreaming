...
JNIEXPORT jlong JNICALL Java_jtux_UUtil_fifo_1init(JNIEnv *env, jclass obj, jlong daddr, 
  jint size, jint semkey, jint to_unlock_empty_sem, int to_unlock_full_sem)
{
       intptr_t  p;
       void* data = (void *)((intptr_t)daddr);
       JTHROW_neg1(p = (intptr_t)ufifo_init(data, size, semkey, to_unlock_empty_sem, to_unlock_full_sem));
       return p;
}

JNIEXPORT void JNICALL Java_jtux_UUtil_fifo_1free(JNIEnv *env, jclass obj, jlong fifo)
{
	PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	ufifo_free(pfifo);
}

JNIEXPORT void JNICALL Java_jtux_UUtil_jaddr_1from_1fifo(JNIEnv *env, jclass obj, 
  jlong fifo, jlong addr, jbyteArray data, jint datasize, jlong sem_empty, jlong sem_full)
{
        void *p;
	sem_t * empty = (sem_t *)((intptr_t)sem_empty);
	sem_t * full = (sem_t *)((intptr_t)sem_full);
        unsigned int left = datasize;
        unsigned int pos = 0;
	...
	if ((p = (*env)->GetByteArrayElements(env, data, NULL)) == NULL)
        ...
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
        while(true){
                copied = ufifo_out(pfifo, p + pos, left, empty, full);
                left -= copied;
		pos += copied;
                if(left <= 0)
                        break;
...
JNIEXPORT void JNICALL Java_jtux_UUtil_jaddr_1to_1fifo(JNIEnv *env, jclass obj,
  jlong fifo, jlong addr, jbyteArray data, jint datasize, jlong sem_empty, jlong sem_full)
{
        void *p;
	sem_t * empty = (sem_t *)((intptr_t)sem_empty);
	...
	if ((p = (*env)->GetByteArrayElements(env, data, NULL)) == NULL)
        ...
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
        while(true){
                copied = ufifo_in(pfifo, p + pos, left, empty, full);
                left -= copied;
		pos += copied;
                if(left <= 0)
                        break;
...
JNIEXPORT jint JNICALL Java_jtux_UUtil_fifo_1in(JNIEnv *env, jclass obj, jlong fifo){
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	return pfifo->in;
}

JNIEXPORT jint JNICALL Java_jtux_UUtil_fifo_1out(JNIEnv *env, jclass obj, jlong fifo){
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	return pfifo->out;
}

JNIEXPORT jint JNICALL Java_jtux_UUtil_fifo_1stat(JNIEnv *env, jclass obj, jlong fifo){
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	return pfifo->stat;
}

JNIEXPORT jint JNICALL Java_jtux_UUtil_fifo_1stat_1empty(JNIEnv *env, jclass obj, jlong fifo){
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	return pfifo->stat_empty;
}

JNIEXPORT jint JNICALL Java_jtux_UUtil_fifo_1stat_1full(JNIEnv *env, jclass obj, jlong fifo){
        PUFIFO pfifo = (PUFIFO)((intptr_t)fifo);
	return pfifo->stat_full;
}
